...
class ListAppointmentComponent extends Component{
    ...
    deleteAppointment(id){
        AppointmentService.deleteAppointment(id).then((res) => {
            this.setState({appointments: this.state.appointments.filter( appointment => appointment.appointment_id!== id)});
        });
    }
/*
    viewAppointment(id){
    ...
    render() {
        return (
            <div><br/>
                <h2 className="text-center">Appointment List</h2><br/>
                <div className="row">
                                ...
                                this.state.appointments.map(
                                    ...
                                    <tr key= {appointment.appointment_id}>
                                       ...
                                       <td>{appointment.date}</td>
                                         <td>
                                            <button onClick = {()=>this.editAppointment(appointment.appointment_id)} className = "btn btn-info">Edit</button>
                                        </td> 
                                        <td>
                                            <button  style = {{marginLeft: "10px"}} onClick = {()=>this.deleteAppointment(appointment.appointment_id)} className = "btn btn-danger">Delete</button>
                                        </td>

                                        <td>
                                            <button  style = {{marginLeft: "10px"}} onClick = {()=>this.viewAppointment(appointment.appointment_id)} className = "btn btn-info">View</button>
                                        </td>
                                     </tr>  
                                )
                            }
