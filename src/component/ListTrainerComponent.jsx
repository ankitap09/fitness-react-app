...
class ListTrainerComponent extends Component{
    ...
    deleteTrainer(id){
        TrainerService.deleteTrainer(id).then((res) => {
            this.setState({trainers: this.state.trainers.filter( trainer => trainer.trainerId!== id)});
        });
    }
                        ...
                        <tbody>
                            {
                                this.state.trainers.map(
                                    ...
                                    <tr key= {trainer.trainerId}>
                                        ...
                                        <td>
                                            <button  style = {{marginLeft: "10px"}} onClick = {()=>this.deleteTrainer(trainer.trainerId)} className = "btn btn-danger">Delete</button>
                                        </td>

                                        <td>
                                            <button  style = {{marginLeft: "10px"}} onClick = {()=>this.viewTrainer(trainer.trainerId)} className = "btn btn-info">View</button>
                                        </td>
                                     </tr>  
                                )
                            }
